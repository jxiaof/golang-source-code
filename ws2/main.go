...
var serverEvents = websocket.Namespaces{
	ns: websocket.Events{
		websocket.OnNamespaceConnected: func(nsConn *websocket.NSConn, msg websocket.Message) error {
			...
			s := "echo : welcome to atms"
			nsConn.Emit("chat", []byte(s))
		...
		"chat": func(nsConn *websocket.NSConn, msg websocket.Message) error {
			...
			fmt.Printf("\n server recv  --------------------- [%s]:[%s]", nsConn.Conn.ID(), string(msg.Body))
			s := "echo : " + string(msg.Body)
			nsConn.Emit("chat", []byte(s))
...
func main() {
	...
	websocketRoute := app.Get("/ws", websocket.Handler(websocketServer, idGen))
